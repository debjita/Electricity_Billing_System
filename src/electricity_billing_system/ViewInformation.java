
package electricity_billing_system;


import javax.swing.*;
import java.awt.*;
import java.sql.*;
import java.awt.event.*;

public class ViewInformation extends JFrame implements ActionListener{
    JButton b1;
    ViewInformation(String meter){
        setBounds(400,100, 550, 450);
        getContentPane().setBackground(new Color(236, 235, 233));
        setLayout(null);
        
        JLabel title = new JLabel("VIEW CUSTOMER INFORMATION");
        title.setBounds(100, 0, 500, 40);
        title.setFont(new Font("Segoe UI", Font.BOLD, 20));
        add(title);
        
        JLabel l1 = new JLabel("Name:");
        l1.setBounds(150, 80, 100, 20);
        l1.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l1);
        
        JLabel l11 = new JLabel();
        l11.setBounds(300, 80, 100, 20);
        l11.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l11);
        
        JLabel l2 = new JLabel("Meter Number:");
        l2.setBounds(150, 110, 150, 20);
        l2.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l2);
        
        JLabel l12 = new JLabel();
        l12.setBounds(300, 110, 100, 20);
        l12.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l12);
        
        JLabel l3 = new JLabel("Address:");
        l3.setBounds(150, 140, 100, 20);
        l3.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l3);
        
        JLabel l13 = new JLabel();
        l13.setBounds(300, 140, 100, 20);
        l13.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l13);
        
        JLabel l4 = new JLabel("City:");
        l4.setBounds(150, 170, 100, 20);
        l4.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l4);
        
        JLabel l14 = new JLabel();
        l14.setBounds(300, 170, 100, 20);
        l14.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l14);
        
        JLabel l5 = new JLabel("State:");
        l5.setBounds(150, 200, 100, 20);
        l5.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l5);
        
        JLabel l15 = new JLabel();
        l15.setBounds(300, 200, 100, 20);
        l15.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l15);
        
        JLabel l6 = new JLabel("Email:");
        l6.setBounds(150, 230, 100, 20);
        l6.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l6);
        
        JLabel l16 = new JLabel();
        l16.setBounds(300, 230, 150, 20);
        l16.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l16);
        
        JLabel l7 = new JLabel("Phone:");
        l7.setBounds(150, 260, 100, 20);
        l7.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l7);
        
        JLabel l17 = new JLabel();
        l17.setBounds(300, 260, 100, 20);
        l17.setFont(new Font("Segoe UI", Font.PLAIN, 16));
        add(l17);
        
        try{
            Conn c = new Conn();
            ResultSet rs = c.s.executeQuery("select * from customer where meter = '"+meter+"'");
            while(rs.next()){
                l11.setText(rs.getString(1));
                l12.setText(rs.getString(2));
                l13.setText(rs.getString(3));
                l14.setText(rs.getString(4));
                l15.setText(rs.getString(5));
                l16.setText(rs.getString(6));
                l17.setText(rs.getString(7));
                
            }
        }catch(Exception e){}
        
        b1 = new JButton("Back");
        b1.setBackground(Color.BLACK);
        b1.setForeground(Color.WHITE);
        b1.setBounds(200, 340, 100, 25);
        b1.addActionListener(this);
        add(b1);
        
        //ImageIcon i1 = new ImageIcon(ClassLoader.getSystemResource("icon/viewinfo.png"));
        //Image i2 = i1.getImage().getScaledInstance(200, 200, Image.SCALE_DEFAULT);
        /*ImageIcon i3 = new ImageIcon(i2);
        JLabel l8  = new JLabel(i3);
        l8.setBounds(20, 350, 600, 300);
        add(l8);*/
    }
    
    public void actionPerformed(ActionEvent ae){
        this.setVisible(false);
    }
    
    public static void main(String[] args){
        new ViewInformation("").setVisible(true);
    }
}
